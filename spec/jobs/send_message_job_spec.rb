...
RSpec.describe SendMessageJob, type: :job do
  ...
  let (:recipient) { create(:recipient, sms_status: sms_status) }
  ...
  describe "when performed" do
    ...
    context "when the recipient status is imported" do
      ...
      it "sends a message with the right body" do
        allow(MessageService).to receive(:new) { message_service }
        expect(message_service).to receive(:send_message).with(recipient, recipient.message_batch.message_template.body)
